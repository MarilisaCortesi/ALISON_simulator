                ...
                for r in F.readlines():
                        if '#' not in r:
                                ...
                                if has_time:
                                        ...
                                        raise ValueError('convert in a generic unit')
                                else:
                                '''
                                value = float(temp[1])
                                out[variable] = value
...
def get_drug_level(configuration, media_volume, scale_factor, mesh_elements):
    print(configuration)
    if 'name' in configuration['drug']:
        drug_amount = (configuration['drug']['dose'][0]*10**6*media_volume)/(scale_factor*mesh_elements) # transform unit in uM

    else:
        drug_amount = {}
        for d in configuration['drug']:
            if not isinstance(configuration['drug'][d],dict):
                continue
            drug_amount[d] = (configuration['drug'][d]['dose']*10**6*media_volume)/(scale_factor*mesh_elements)
    print('scale factor drug', drug_amount)
    return drug_amount
...
def get_glucose_level(media, scale_factor, mesh_elements):
        media_type = media[0]
        media_volume = utility.engineering_notation(media[1].split('l')[0])
        full_path = os.getcwd() + os.path.sep + 'media' + os.path.sep + media_type + '.txt'
        with open(full_path) as F:
                for r in F.readlines():
                        if 'glucose' in r:
                                level_gml = float(r.split('->')[1].split('[')[0])  # ug/l
                                break
        gculture = (media_volume * level_gml)/scale_factor
        return gculture / mesh_elements


def get_initial_conditions(configuration, scale_factor, mesh_elements):
        ...
        output_fixed_flux = 0
        output_initial_conditions['oxygen'] = get_oxygen_level(configuration, scale_factor, mesh_elements)
        output_initial_conditions['glucose'] = get_glucose_level(configuration['media'], scale_factor, mesh_elements)
        if configuration['treatment'] != 'none':
                output_initial_conditions['drug'] = get_drug_level(configuration['treatment'],utility.engineering_notation(configuration['media'][1].split('l')[0]),scale_factor, mesh_elements)
        return output_initial_conditions, output_fixed_flux


def get_oxygen_level(configuration, scale_factor, mesh_elements):
        culture_volume = utility.engineering_notation(configuration['media'][1])
        o2_level = configuration['oxygen']
        if o2_level == 'standard':
                o2_density = 1241000  #[ug/l]  at a temperature of 37oC and a pressure of 1 bar
        else:
                o2_density = 1241000 * float(o2_level) / 20  # the standard O2 level in an incubator is 20%
        total_amount = (culture_volume * o2_density)/scale_factor  # see notebook 3 for the calculation

        return total_amount / mesh_elements


def initialise_f(mesh, fields, cells_population,vector_space):
        output_f = {}
        for ic in fields:
                if isinstance(fields[ic], dict):
                    for icc in fields[ic]:
                        drug = icc.split('_')[0]
                        ...
                        for c in cells_population:
                            temp[c.location] = get_rate(c, drug)
                        output_f[drug] = fenics.Function(vector_space)
                        output_f[drug].vector().set_local(temp)
                else:
                        temp = np.zeros((len(mesh.coordinates()[:, 0]))) # from the example in fenics it looks like f has the same length as the number of nodes
                        for c in cells_population:
...
def update_f(fields, cells_population,vector_space):
        ...
        for ic in fields:
                ...
                output_f[ic].vector().set_local(temp)
        return output_f
def initialise_mesh(flnm):
        full_name = os.getcwd() + os.path.sep + 'meshes' + os.path.sep + flnm
        mesh = dolfin.Mesh()
        with dolfin.XDMFFile(full_name) as F:
                F.read(mesh)
        #cylinder = mshr.Cylinder(fenics.Point(0, 0, 0), fenics.Point(0, 0, 1.875), 0.625, 0.625) # these are the standard dimensions of a 96 well plate in scale 1:8
        #geometry = cylinder
                        ...
                        if cell.rules['current_rules']['environment'][r]['variable'] == field:
                                ...
                                try:
                                        ...
                                        if par in cell.parameters:
                                                ...
                                                rate = utility.solve_equation(par, cell.parameters)
...
def initialise_fields(initial_conditions, function_space):
        out = {}
        for i in initial_conditions:
                if isinstance(initial_conditions[i], dict):
                    for d in initial_conditions[i]:
                        drug = d.split('_')[0]
                        temp = fenics.Expression((str(initial_conditions[i][d])),degree=1)
                        out[drug] = fenics.interpolate(temp, function_space)

                else:
                    temp = fenics.Expression((str(initial_conditions[i])), degree=1)
...
def boundary(x, on_boundary):
        tol = 1E-14
        return on_boundary and fenics.near(x[0-1], 1.1, tol) #TODO get the top of the well automatically
...
def set_boundary_conditions(initial_conds, function_space):
        out = {}
        max_z = max(function_space.mesh().coordinates()[:, -1])
        for i in initial_conds:
                if isinstance(initial_conds[i], dict):
                    for ii in initial_conds[i]:
                        drug = ii.split('_')[0]
                        out[drug] = fenics.DirichletBC(function_space, initial_conds[i][ii], boundary)
                else:
...
def solve_diffusion(precomputed_pars, field_predictor, boundary_condition, init_cond, field, cell_population):
